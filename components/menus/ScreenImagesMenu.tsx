...
import { Search, ChevronDown, ChevronRight, Layout, Smartphone, Tablet, Monitor, ChevronsUp, ChevronsDown, Eye, Pin } from 'lucide-react';
import { SCREEN_IMAGES } from '../../data/screen_images/index';
import { ScreenImage } from '../../types';

interface ScreenImagesMenuProps {
...
export const ScreenImagesMenu: React.FC<ScreenImagesMenuProps> = ({ onPreviewImage, isPinned, onTogglePin }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [deviceFilter, setDeviceFilter] = useState<DeviceFilter>('all');
  
  const categoriesList = useMemo(() => Array.from(new Set(SCREEN_IMAGES.map(t => t.category))), []);
                        ...
                        {expandedCategories.includes(category) && (
                            <div className="p-3 bg-white dark:bg-gray-900 grid grid-cols-2 gap-3">
                                {categoryImages.map(image => (
                                    <div 
                                        key={image.id} 
                                        className="group relative border border-gray-200 dark:border-gray-600 rounded-lg overflow-hidden hover:shadow-md transition-all cursor-pointer aspect-[9/16]"
