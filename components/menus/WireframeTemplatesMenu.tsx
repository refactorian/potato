...
import { Search, ChevronDown, ChevronRight, Layout, Smartphone, Tablet, Monitor, ChevronsUp, ChevronsDown, PenTool, Eye, Plus, Pin } from 'lucide-react';
import { ALL_WIREFRAMES } from '../../data/wireframes/index';
import { Project, Screen, TemplateDefinition } from '../../types';
import { v4 as uuidv4 } from 'uuid';

interface WireframeTemplatesMenuProps {
    project: Project;
...
export const WireframeTemplatesMenu: React.FC<WireframeTemplatesMenuProps> = ({ project, setProject, onPreviewTemplate, isPinned, onTogglePin }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [deviceFilter, setDeviceFilter] = useState<DeviceFilter>('mobile');
  
  const categoryList = useMemo(() => Array.from(new Set(ALL_WIREFRAMES.map(t => t.category))), []);
  const [expandedCategories, setExpandedCategories] = useState<string[]>(categoryList);
  ...
  const toggleAllCategories = () => {
    if (expandedCategories.length === categoryList.length) {
        setExpandedCategories([]);
    } else {
        setExpandedCategories(categoryList);
    }
  };
  ...
  const filteredTemplates = useMemo(() => {
      return ALL_WIREFRAMES.filter(t => {
          const matchesSearch = t.name.toLowerCase().includes(searchQuery.toLowerCase());
          const navbar = t.elements.find(e => e.name.toLowerCase().includes('header') || e.type === 'navbar');
  ...
  const applyTemplate = (templateId: string) => {
    const template = ALL_WIREFRAMES.find(t => t.id === templateId);
    if (!template) return;
  ...
  const allExpanded = expandedCategories.length === categoryList.length;
            ...
            {categoryList.map((category) => {
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="h-20 bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-400 dark:text-gray-500 group-hover:bg-indigo-50 dark:group-hover:bg-gray-600 group-hover:text-indigo-400 transition-colors">
                                                {(LucideIcons as any)[template.thumbnail] ? React.createElement((LucideIcons as any)[template.thumbnail], {size: 24}) : <PenTool size={24} />}
             ...
             {filteredTemplates.length === 0 && (
                <div className="text-center py-8 text-gray-400 text-xs">
                    No wireframes found matching your filters.
                </div>
             )}
        </div>
    </div>
